#!/usr/bin/env python3
"""Generate MOOP Dictionary entries from a JSON list.

Usage:
    python3 bin/generate_dict_entries.py path/to/entries.json

Each record (one per dictionary entry) must include these keys:
    slug, word, pronunciation, pos, etymology,
    biblical_def, webster_summary, webster_full,
    scriptures (list of [ref, verse-text]),
    corruption_summary, corruption_paragraphs (list of strings),
    roots_summary, roots_lines (list of HTML-string lines),
    usage (list of three strings),
    related (list of [slug, label]).
...
# Template uses ${TOKEN} markers (string.Template syntax) so the CSS/JS curly
# braces inside the body don't collide with f-string / format() escaping.
...
<head>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .section-summary { color:var(--gray); font-size:0.92rem; font-style:italic; margin-bottom:4px; }
        ...
        .dict-back-nav { display:flex; align-items:center; justify-content:center; flex-wrap:wrap; gap:8px; margin-bottom:22px; padding:10px 0; border-bottom:1px solid var(--border,#333); }
        .dict-back-nav a { color:var(--gold,#D4AF37); text-decoration:none; font-size:0.88rem; font-weight:500; }
...
<body>
    ...
    <div class="container">
        <div class="dict-back-nav"><a href="index.html">&#8592; Back to Dictionary</a></div>
        ...
        <div class="section">
            <h3><img src="../assets/icons/shield-blog-quill-48.png" width="20" height="20" style="vertical-align:middle;margin-right:6px;"> Usage</h3>
            ${USAGE}
        </div>
    ...
    <footer>
        ...
        <p style="margin-top:8px;font-size:0.78rem;color:#555;">Powered by MOOPbot Pro &mdash; &ldquo;Iron sharpens iron.&rdquo; &mdash; <a href="../bible.html?ref=Proverbs+27:17" style="color:var(--gold);text-decoration:underline;">Proverbs 27:17</a></p>
        <p style="margin-top:6px;font-size:0.72rem;color:#666;">MOOP Dictionary <strong style="color:var(--gold);">V4.0</strong> &middot; Multi-Generational Edition &middot; <a href="index.html" style="color:#666;">index</a></p>
    </footer>
    <script>
    function toggleTheme(){
        var b=document.body,t=document.getElementById('themeToggle');
        if(b.classList.contains('light-mode')){b.classList.remove('light-mode');t.textContent='\u{1F319}';localStorage.setItem('bte-theme','dark');}
    ...
    document.querySelectorAll('details').forEach(function(d){
        d.addEventListener('toggle', function(){
            var sum = d.previousElementSibling;
            if(sum && sum.classList.contains('section-summary')){
                sum.style.display = d.open ? 'none' : '';
            }
        });
    });
    </script>
...
def render_scriptures(scriptures):
    parts = []
    for ref, text in scriptures:
        url = ref.replace(' ', '+')
        parts.append(
            f'<p><a href="../bible.html?ref={url}" class="verse-ref">{ref}</a> &mdash; <em>"{text}"</em></p>'
        )
    return '\n            '.join(parts)


def render_paragraph_block(paragraphs):
...
def render(record):
    return HTML_TEMPLATE.substitute(
        ...
        CORRUPTION_SUMMARY=record['corruption_summary'],
        CORRUPTION_FULL=render_paragraph_block(record['corruption_paragraphs']),
        ROOTS_SUMMARY=record['roots_summary'],
...
def main():
    if len(sys.argv) < 2:
        print('Usage: generate_dict_entries.py <entries.json>', file=sys.stderr)
        sys.exit(1)
    with open(sys.argv[1], 'r', encoding='utf-8') as f:
        records = json.load(f)
    written = 0
    for r in records:
        target = os.path.join(DICT_DIR, f"{r['slug']}.html")
        with open(target, 'w', encoding='utf-8') as out:
            out.write(render(r))
        written += 1
        print(f"wrote {r['slug']}.html")
    print(f"\n{written} entries written to {DICT_DIR}")
