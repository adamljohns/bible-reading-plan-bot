...
TOOLS = <repo>/tools/worship-sprint   (this dir; REPO-COMMITTED — never keep loop assets in /tmp scratchpads, they get wiped)
...
2. Wave via Workflow tool, scriptPath=TOOLS/worship-sprint.js:
   ...
   - else new wave: args {round:<hour>, cats: next STATE.waveSize of TOOLS/category-bank.json from catIndex} as [{"type":"cat","name":...}]; catIndex+=waveSize; bank exhausted → reuse categories + " — second pass, go even deeper".
...
4. Apply on completion: `node TOOLS/apply-round.js <task .output path> TOOLS/reports/hymns-<round>.json`
   - gap = target − count; extracted > gap → `cd WORKTREE && node TOOLS/trim-to-gap.js <hymns> <gap> <out>` and
     append the .surplus.json entries to WORKTREE/data/worship-hymn-bank.json (dedup by slugified title).
   - `node scripts/assemble-worship-additions.js /dev/null <hymns file>` — NOTE: pass TOOLS/reports/empty.json ([]) not /dev/null.
...
- **chord-lint**: `node TOOLS/chord-lint.js` writes TOOLS/reports/chord-lint.json (garbled chord lines, encoding
...
- **lyrics-fidelity** (needs subagents): sample 40 agent-added hymns (ext:'hymn', rotate via STATE.verifyCursor),
  ...
  entries reported unfaithful (publicDomain:false) → fix from source or remove via worship-purged.json; log in TOOLS/reports/.
...
- Fresh CCM batches: add entries to a JSON list and run TOOLS/build-linksonly.js (see header) — genre-tag
