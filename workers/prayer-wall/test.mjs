...
/* --- 1. unconfigured deploy must refuse everyone, not admit everyone --- */
{
  ...
  const r = await worker.fetch(req('/api/prayer/requests'), env, ctx);
  ok('unconfigured -> 503 not_configured', r.status === 503, `got ${r.status}`);
  const r2 = await worker.fetch(
    req('/api/prayer/session', { method: 'POST', body: { pin: 'anything' } }), env, ctx);
...
/* --- 2. no session = no data --- */
{
  ...
  ok('401 body carries no requests array', body.requests === undefined);
}
...
/* --- 3. wrong PIN --- */
{
  ...
  const r = await worker.fetch(
    req('/api/prayer/session', { method: 'POST', body: { pin: 'members-1235' } }), env, ctx);
...
/* --- 4. brute force brake --- */
{
  ...
  for (let i = 0; i < 12; i++) {
    last = await worker.fetch(
      req('/api/prayer/session', { method: 'POST', body: { pin: 'nope' + i } }), env, ctx);
  }
...
{
  const r = await worker.fetch(
    req('/api/prayer/session', { method: 'POST', body: { pin: 'members-1234' } }), envShared, ctx);
  ...
  const g = await worker.fetch(req('/api/prayer/requests', { cookie: memberCookie }), envShared, ctx);
  ok('with session -> 200 on GET requests', g.status === 200, `got ${g.status}`);
  const gb = await g.json();
  ok('empty wall returns an empty list, not an error', Array.isArray(gb.requests) && gb.requests.length === 0);
...
/* --- 6. forged / tampered cookies --- */
{
  ...
  const r = await worker.fetch(req('/api/prayer/requests', { cookie: forged }), envShared, ctx);
  ...
  const r2 = await worker.fetch(req('/api/prayer/requests', { cookie: escalated }), envShared, ctx);
  ...
  const r3 = await worker.fetch(req('/api/prayer/requests', { cookie: expired }), envShared, ctx);
...
let postedId;
{
  const r = await worker.fetch(req('/api/prayer/requests', {
  ...
  const long = await worker.fetch(req('/api/prayer/requests', {
    method: 'POST', cookie: memberCookie,
  ...
  const empty = await worker.fetch(req('/api/prayer/requests', {
...
{
  const s = await worker.fetch(req(`/api/prayer/requests/${postedId}/status`, {
    method: 'POST', cookie: memberCookie, body: { status: 'answered', praise: 'nope' },
  }), envShared, ctx);
  ok('member marking answered -> 403', s.status === 403, `got ${s.status}`);

  const d = await worker.fetch(req(`/api/prayer/requests/${postedId}`, {
    method: 'DELETE', cookie: memberCookie }), envShared, ctx);
  ok('member deleting -> 403', d.status === 403, `got ${d.status}`);

  const e = await worker.fetch(req('/api/prayer/export', { cookie: memberCookie }), envShared, ctx);
...
/* --- 9. moderator can --- */
{
  const m = await worker.fetch(
    req('/api/prayer/session', { method: 'POST', body: { pin: 'moder-9876' }, ip: '5.6.7.8' }),
    envShared, ctx);
  ok('moderator PIN -> 200', m.status === 200, `got ${m.status}`);
  const modCookie = cookieFrom(m);

  const s = await worker.fetch(req(`/api/prayer/requests/${postedId}/status`, {
    method: 'POST', cookie: modCookie, body: { status: 'praying' },
  }), envShared, ctx);
  ok('moderator can mark praying', s.status === 200, `got ${s.status}`);

  const a = await worker.fetch(req(`/api/prayer/requests/${postedId}/status`, {
    method: 'POST', cookie: modCookie, body: { status: 'answered', praise: 'He gave the words.' },
  }), envShared, ctx);
  ...
  ok('answered stamps answered_at', Boolean(ab.request.answered_at));

  const bad = await worker.fetch(req(`/api/prayer/requests/${postedId}/status`, {
    method: 'POST', cookie: modCookie, body: { status: 'deleted-by-me' },
  }), envShared, ctx);
  ok('invalid status rejected -> 400', bad.status === 400, `got ${bad.status}`);

  const e = await worker.fetch(req('/api/prayer/export', { cookie: modCookie }), envShared, ctx);
  ...
  const del = await worker.fetch(req(`/api/prayer/requests/${postedId}`, {
    method: 'DELETE', cookie: modCookie }), envShared, ctx);
...
/* --- 10. submit rate limit --- */
{
  ...
  const s = await worker.fetch(
    req('/api/prayer/session', { method: 'POST', body: { pin: 'members-1234' } }), env, ctx);
  const c = cookieFrom(s);
  let last;
  for (let i = 0; i < 8; i++) {
    last = await worker.fetch(req('/api/prayer/requests', {
...
{
  const r = await worker.fetch(req('/api/prayer/session', { method: 'DELETE', cookie: memberCookie }),
                               envShared, ctx);
